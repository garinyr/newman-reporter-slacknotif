...
function slackMessage(stats, timings, failures, executions, maxMessageSize, environment, channel, reportingUrl, projectName, featureName, limitFailures) {
  ...
  return jsonminify(`
    {
        ...
        "blocks": [
            ...
            ${collectionAndEnvironentFileBlock(projectName, featureName, environment)}
...
function collectionAndEnvironentFileBlock(projectName, featureName, environment) {
  if (projectName == '') {
    return `{
            "type": "section",
			"text": {
  ...
  } else if (projectName && featureName) {
    return `{
        "type": "section",
      "text": {
        "type": "mrkdwn",
...
module.exports = { slackMessage, send }